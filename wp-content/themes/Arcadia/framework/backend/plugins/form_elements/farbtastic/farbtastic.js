 ...
 * © 2008 Steven Wittens
 *
 * Modified by Ruven Pelka for use in the WordPress Backend
 * - UPDATED: All '$' have been replaced with 'jQuery'
 * - ADDED: "jQuery(fb.callback).change();" to line 255 to make "change" event work
 *
  ...
  fb.updateDisplay = function () {
    ...
    if (typeof fb.callback == 'object') {
      ...
      jQuery(fb.callback).each(function() {
        if (this.value && this.value != fb.color) {
          this.value = fb.color;
          jQuery(fb.callback).change(); // EDITED
        }
      });
    }
